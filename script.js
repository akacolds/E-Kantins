...
// Helper Notifikasi Suara & Pop-up Browser
function membunyikanNotif() {
...
_supabase
  ...
  .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'orders_kantin' }, async (payload) => {
      ...
      if (currentUser && currentUser.role === 'kantin' && parseInt(currentUser.kantinId, 10) === parseInt(newOrder.kantin_id, 10)) {
          membunyikanNotif();
          kirimNotifBrowser("🔔 Pesanan Baru Masuk!", `${newOrder.nama_pemesan} memesan: ${newOrder.nama_menu} (${newOrder.qty} Porsi)`);
      }

      orderData = await loadOrders();
      refreshUI();
  })
  .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'orders_kantin' }, async (payload) => {
      ...
      if (newChats.length > oldChatCount) {
          const pesanTerakhir = newChats[newChats.length - 1];

          if (currentUser && currentUser.role === 'murid' && updatedOrder.user_key === currentUser.userKey && pesanTerakhir.sender === 'kantin') {
              membunyikanNotif();
              kirimNotifBrowser(`💬 Chat dari ${getNamaKantinById(updatedOrder.kantin_id)}`, pesanTerakhir.text);
          }

          if (currentUser && currentUser.role === 'kantin' && parseInt(currentUser.kantinId, 10) === parseInt(updatedOrder.kantin_id, 10) && pesanTerakhir.sender === 'murid') {
              membunyikanNotif();
              kirimNotifBrowser(`💬 Chat dari ${updatedOrder.nama_pemesan}`, pesanTerakhir.text);
          }
      }
      ...
      if (oldOrder && oldOrder.status !== updatedOrder.status) {
          if (currentUser && currentUser.role === 'murid' && updatedOrder.user_key === currentUser.userKey) {
              membunyikanNotif();
              kirimNotifBrowser("🔄 Update Pesanan!", `Status pesanan ${updatedOrder.nama_menu}: "${updatedOrder.status}"`);
          }
      }
...
function renderKatalogPembeli() {
    ...
    if (items.length === 0) {
        grid.innerHTML = `<p class="subtitle">Belum ada menu tersedia.</p>`;
        return;
    }
    ...
    items.forEach(item => {
        ...
        card.innerHTML = `
            ...
            <div class="menu-content">
                ...
                <button type="button" class="btn btn-primary btn-block" ${habis ? 'disabled' : ''} onclick="bukaModalPesan(${item.id})">
...
function bukaModalPesan(itemId) {
    ...
    document.getElementById('modal-kantin-menu').innerText = `${itemDipilih.namaKantin} • Bayar Langsung di Kantin`;
...
async function konfirmasiKirimPesanan() {
    ...
    membunyikanNotif();
    kirimNotifBrowser("✅ Pesanan Berhasil!", `Pesanan ${itemDipilih.nama} berhasil dikirim ke ${itemDipilih.namaKantin}.`);
...
async function renderPesananPembeli() {
    ...
    if (myOrders.length === 0) {
        list.innerHTML = `<p class="subtitle">Belum ada riwayat pesanan.</p>`;
        return;
    }
    ...
    myOrders.forEach(o => {
        ...
        let chatHTML = (o.chats && o.chats.length > 0) ? o.chats.map(c => `
            <div class="chat-bubble ${c.sender === 'murid' ? 'chat-self' : 'chat-other'}">
                ...
                <div>${c.text}</div>
            </div>`).join('') : `<span style="font-size:11px;color:#94a3b8;">Belum ada pesan.</span>`;

        card.innerHTML = `
            <div class="order-top">
                <div>
                    <strong>${o.namaMenu} (${o.qty} Porsi)</strong> - ${formatRupiah(o.harga)}<br>
                    <small class="subtitle">Kantin: ${getNamaKantinById(o.kantinId)} • Waktu: ${o.waktu}</small><br>
            ...
            <div class="chat-section">
                <div class="chat-toggle-title">💬 Obrolan Realtime</div>
...
async function renderPesananKantin() {
    ...
    const masuk = orderData.filter(o => parseInt(o.kantinId, 10) === targetKantinId && !isSudahSelesaiSatuMenit(o));
    ...
    if (masuk.length === 0) {
        list.innerHTML = `<p class="subtitle">Belum ada pesanan masuk.</p>`;
        return;
    }
    ...
    masuk.forEach(o => {
        ...
        let chatHTML = (o.chats && o.chats.length > 0) ? o.chats.map(c => `
            <div class="chat-bubble ${c.sender === 'kantin' ? 'chat-self' : 'chat-other'}">
                ...
                <div>${c.text}</div>
            </div>`).join('') : `<span style="font-size:11px;color:#94a3b8;">Belum ada obrolan.</span>`;
...
async function renderPesananSelesaiKantin() {
    ...
    const targetKantinId = parseInt(currentUser.kantinId, 10);
    const selesai = orderData.filter(o => parseInt(o.kantinId, 10) === targetKantinId && isSudahSelesaiSatuMenit(o));

    if (selesai.length === 0) {
        list.innerHTML = `<p class="subtitle">Belum ada pesanan yang selesai lebih dari 1 menit.</p>`;
        return;
    }
...
function renderMenuKantin() {
    ...
    if (myMenu.length === 0) {
        grid.innerHTML = `<p class="subtitle">Belum ada menu.</p>`;
        return;
    }
