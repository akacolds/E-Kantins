...
async function initAppData() {
    // Minta izin notifikasi browser saat aplikasi dibuka
...
_supabase
  ...
  .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'orders_kantin' }, async (payload) => {
      ...
      // Cek apakah akun yang sedang login adalah Kantin & pesanan baru ini ditujukan untuk kantin tersebut
      if (currentUser && currentUser.role === 'kantin' && parseInt(currentUser.kantinId, 10) === parseInt(newOrder.kantin_id, 10)) {
          ...
          // 2. Tampilkan Pop-up Notifikasi Browser
          if ("Notification" in window && Notification.permission === "granted") {
